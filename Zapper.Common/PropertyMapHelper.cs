...
namespace Zapper.Common
{
    public static class PropertyMapHelper
    {
        ...
        {
            // get column names - probably a better way to do this
            var columnNames = type.GetProperties();

            foreach (var column in columnNames)
            {
                if (!String.IsNullOrWhiteSpace(column.ToString()) && row.Table.Columns.Contains(column.ToString()))
                {
                    var propValue = row[column.ToString()];

                    if (propValue != DBNull.Value)
                    {
                        ParsePrimitive(prop, entity, row[column.ToString()]);
                        break;
                    }
                }
            }
